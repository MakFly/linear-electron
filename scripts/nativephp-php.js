...
// PATCHED (see project Makefile `electron-doctor`): the original async yauzl
// extraction raced with process exit and produced a corrupted, non-executable
// binary that macOS Gatekeeper killed (SIGKILL). Extract synchronously instead.
if (platform.phpBinary) {
    try {
        ...
        if (isWindows) {
            execSync(`powershell -Command "Expand-Archive -Force '${binarySrcDir}' '${binaryDestDir}'"`);
        } else if (isDarwin) {
            execSync(`ditto -x -k "${binarySrcDir}" "${binaryDestDir}"`);
        } else {
            execSync(`unzip -o "${binarySrcDir}" -d "${binaryDestDir}"`);
        }
